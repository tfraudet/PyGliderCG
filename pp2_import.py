...
def cli(filename):
	...
	def datum (planref, typeappui):
		if planref == 1 and typeappui == 1:
			return 1
		elif planref == 1 and typeappui == 2:
			return 2
		elif planref == 1 and typeappui == 3:
			return 3
		else:
			return 4
	...
	for index, row in df.iterrows():
		...
		conn.execute('INSERT INTO GLIDER VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',[
			...
			# Datum for weighings
			datum(row['PlanRef'], row['TypeAppui']),	# datum 
			row['PosPil'],								# pilot_position 
			row['PlanRefTxt'],							# datum_label 
			row['CalleRef'], 							# wedge 
			row['PosCalRef'],							# wedge_position 
			...
			row['MasseMiniPil'],						# weight_min_pilot
		...
		for equ_idx, equipement in inventory_df.iterrows():
			sql = "INSERT INTO INVENTORY VALUES (nextval('inventory_id_seq'), '{}', {}, '{}', '{}', '{}', '{}', {}, '{}')".format(
				row['Immat'],									# registration
			...
			conn.execute(sql)
			# conn.execute('INSERT INTO INVENTORY VALUES (id, registration, on_board, instrument, brand, type, number, date, seat)',[
			# 	next_id,
			# 	row['Immat'],									# registration
			# 	equipement['On-Board'],							# on_board
			# 	equipement['Instrument'],						# instrument
